...
**Delivery is deliberately not automatic.** `Spool.Status` — the queue every git hook and agent turn drains — walks `refs/sessions/*` only. `refs/sync/*` is excluded on purpose: a session has one writer and a linear history, so a diverged push is a real conflict worth refusing; the sync ref does not — every machine that has ever run `rgt init`/`rgt connect` or fired a git hook writes its own rootless sync chain with no shared ancestor, so two ordinary machines "diverge" on it as their normal starting state. Folding it into the automatic queue once made that ordinary state surface as perpetual pending status, failed pushes, and refused pulls between machines that had done nothing wrong. Delivering it stays possible, just explicit: `rgt sync --workspace` writes the baseline and then pushes exactly that ref, the same way `rgt sync <ref>` pushes any other named ref (`rgt pull sync/workspace` and `rgt sync --repair sync/workspace` work the same way, targeted). See the comment on `Spool.Status` (`internal/remote/spool.go`) for the incident this shape guards against.
