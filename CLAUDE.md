...
4. **Ref** — the only mutable object. A named pointer (e.g. `refs/sessions/<session_id>`) holding a step hash. `refs/sync/workspace` is the one ref outside the `sessions/` namespace: it holds the tip of the workspace-sync baseline chain (see "Workspace sync baseline" below), and is deliberately never enumerated as a session.
...
### Workspace sync baseline

A tree only otherwise exists as a step snapshot, so a project's Files view and blame are empty until the first captured agent step — which can be a long wait on a repository an agent barely touches. The workspace sync baseline (issue #106) fixes this: `rgt init` and `rgt connect` take one snapshot of the git working tree right after hooks are wired, `rgt sync --workspace` refreshes it on demand, and a `post-commit` git hook refreshes it automatically (a `pre-push` hook does the same, ahead of its own job of draining the server-mode delivery queue). All of these call `capture.WorkspaceSync`, which is idempotent: an unchanged tree writes nothing.

Mechanically, a workspace sync is a normal `store.Step` — `Origin: "sync"`, `SessionID: "sync:workspace"`, no causes, transcript, or conversation — chained onto the single ref `refs/sync/workspace` (a new ref directory, `sync`, parallel to `sessions`). It is deliberately not a session: `rgt sessions`/`/api/sessions` enumerate `refs/sessions/*` only, and this ref is never in that set.

Two things read it:

- **Files** (`GET /api/files` with no `step`/`session` params) resolves the newest tree across every `sessions/*` and `sync/*` ref tip by `TimestampNanos` (ties favor a session), so the view is never empty before an agent runs — and stays populated in between whenever an agent has been idle since its last turn but the working tree kept moving via plain commits.
- **Blame** seeds from it. When `computeBlameForStep` (capture, write-time) or `deriveBlame` (server, query-time — see the two-path table above) has no parent-tree entry for a path — the first step of a session, most commonly — both consult the current `refs/sync/workspace` tip and use its content/blame as the "old" side, so unmodified baseline lines stay attributed to the sync step instead of being swept into the agent's first step. A project with no sync ref behaves exactly as it did before this existed. `handleAPIBlame` needed no special-casing for a sync step's own blame: it already reports whatever `Origin` the step carries, so a sync step's lines read `origin: "sync"` for free.

**Delivery is never folded into `Spool.Status`, but it does happen automatically.** `Spool.Status` — the queue every git hook and agent turn drains — walks `refs/sessions/*` only, and `refs/sync/*` is excluded on purpose: a session has one writer and a linear history, so a diverged push is a real conflict worth refusing; the sync ref does not, on its own, have that property — every machine that has ever run `rgt init`/`rgt connect` or fired a git hook would otherwise write its own rootless sync chain with no shared ancestor, and folding that into the automatic queue once made two machines that had done nothing wrong surface as perpetual pending status, failed pushes, and refused pulls (see the comment on `Spool.Status`, `internal/remote/spool.go`, for that incident).

The actual fix is convergence, not silence: before writing, a server-mode sync (`capture.WorkspaceSyncOnto`, driven by `resolveServerSyncParent` in `internal/cli/workspacesync.go`) asks the server for its current `refs/sync/workspace` tip and, when one exists, fetches just that step and its tree (`remote.FetchObject`, never the tree's blobs) and chains the new step's `Parent` onto it — so every machine's sync chain converges onto one shared lineage instead of each rooting an incompatible one, and the CAS conflicts the old design produced mostly disappear by construction. The *local* ref's own compare-and-swap still checks against whatever the local ref actually held, never the server hint, which is why a mismeasured or raced hint can only affect which lineage the next step records itself onto, never the safety of the local write.

Delivery itself is an explicit, targeted push of `refs/sync/workspace` by name (`remote.Push`, the same call any `rgt sync <ref>` makes) — never a side effect of Spool.Status/Flush — made automatically right after every write that produced one: by `rgt sync --workspace`'s command path, by `rgt init`/`rgt connect`'s first-run baseline (`runBaselineSync` → `deliverWorkspaceSyncServerMode`), and by the post-commit and pre-push git hooks (through the same function, bounded and silent on failure — see `boundedRun`'s doc comment on what "bounded" can and cannot promise for a process this short-lived). A push that still loses a genuine race — another machine advanced the tip between our lookup and our push — is retried once against the now-current tip; a second collision is left for the next natural trigger. `rgt pull sync/workspace` and `rgt sync --repair sync/workspace` remain available for explicit, targeted use outside these automatic paths.

One more asymmetry worth naming: blame sidecars are derived data and are never transferred to a remote server (see the two-path table above) — only steps, trees, and blobs are, and only when the sync ref was actually pushed. So a genuinely remote server can seed content from the sync step's tree and blob once delivered, but only recovers the *stored* sync-step blame map when capture and the server share a disk (self-hosted, single machine). Otherwise `deriveBlame` still seeds correctly by content when the baseline blob matches exactly, and falls back to attributing to the step being blamed when it does not.
...
| **Workspace sync** | A step recorded outside any agent turn (`Origin: "sync"`), snapshotting the git working tree. Chains onto `refs/sync/workspace`, never a session ref. See "Workspace sync baseline" above. |
