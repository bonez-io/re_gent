...
export function SyncPanel({ state = 'connected' }: SyncPanelProps) {
  ...
  return <section className="mx-auto w-full max-w-[720px] p-5">
    ...
    <div className="overflow-hidden rounded-[8px] border border-line">{rows.map(([label, value]) => <div key={label} className="grid grid-cols-[120px_minmax(0,1fr)] border-b border-line text-[10.5px] last:border-b-0"><div className="bg-inset px-3 py-2 text-ink-3">{label}</div><div className="px-3 py-2 font-mono text-ink-2">{value}</div></div>)}</div>
