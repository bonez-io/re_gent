...
export function CodeBlock({ filename, language, code }: CodeBlockProps) {
  ...
  return <div className="w-full overflow-hidden rounded-[7px] border border-line bg-inset transition-colors duration-150 hover:border-ink-3/40">
    ...
    <pre className="m-0 overflow-auto bg-inset py-1.5 font-mono text-[10px] leading-[1.6]">{code.split('\n').map((line, index) => <div key={`${index}-${line}`} className="flex px-2"><span className="w-5 shrink-0 select-none text-right text-[9px] text-ink-3/60">{index + 1}</span><code className="pl-2.5 text-ink-2">{line || ' '}</code></div>)}</pre>
  </div>
}
