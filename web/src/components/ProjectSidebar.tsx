...
export function ProjectSidebar({ project = 'girlfriend-assistant', deployment = 'Self-hosted · local', conversationCount = 7, active: controlled, onNavigate, onProjectClick }: ProjectSidebarProps) {
  ...
  return <aside className="flex h-full min-h-screen w-48 flex-col border-r border-line bg-canvas" aria-label="Project navigation">
    <div className="flex h-11 items-center border-b border-line px-3">
      <a href="https://www.re-gent.dev/" className="flex size-7 items-center justify-center rounded-[6px] transition-[transform,opacity] duration-150 hover:opacity-80 active:scale-95" aria-label="Visit re_gent"><img src="/favicon.svg" alt="" className="block size-7" /></a>
      <button className="ml-auto rounded-[6px] px-1 text-[15px] text-ink-3 transition-colors duration-150 hover:bg-hover hover:text-ink" aria-label="Project menu">•••</button>
    </div>
    <div className="px-2 pt-2">
      <button onClick={onProjectClick} className="flex w-full items-center gap-2 rounded-[7px] px-1.5 py-1 text-left transition-colors duration-150 hover:bg-hover">
        <span className="flex size-6 items-center justify-center rounded-[6px] bg-field font-mono text-[10px] text-accent-ink shadow-hairline">ga</span>
    ...
    <nav className="mt-3 px-2" aria-label="Workspace">
      <div className="mb-1 px-1.5 text-[10.5px] font-medium text-ink-3">Workspace</div>
      {items.map((item) => <button key={item.key} type="button" onClick={() => navigate(item.key)} aria-current={active === item.key ? 'page' : undefined} className={`mb-px flex h-7.5 w-full items-center gap-2 rounded-[7px] px-1.5 text-left transition-[background-color,color,transform] duration-150 active:scale-[0.985] ${active === item.key ? 'bg-hover-2 text-ink shadow-hairline' : 'text-ink-2 hover:bg-hover hover:text-ink'}`}>
    ...
    <div className="mt-auto border-t border-line p-2">
      <div className="flex items-center gap-2 rounded-[5px] px-1.5 py-1 text-[11.5px] text-ink-3">
