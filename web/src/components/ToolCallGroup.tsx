...
export function ToolCallGroup({ calls, files = [], defaultOpen = true }: ToolCallGroupProps) {
  ...
  return <div className="w-full pb-0.5">
    <button type="button" aria-expanded={open} onClick={() => setOpen((value) => !value)} className="-mx-1 flex w-fit items-center gap-1 rounded-[4px] px-1 py-0.5 text-[10.5px] text-ink-3 transition-colors hover:bg-hover-2 hover:text-ink-2">
    ...
    <div className="grid transition-[grid-template-rows,opacity] duration-300" style={{ gridTemplateRows: open ? '1fr' : '0fr', opacity: open ? 1 : 0 }}><div className="min-h-0 overflow-hidden">
      <div className="mt-1 flex flex-col gap-px border-l border-line pl-2.5">
        {calls.map((call) => { const rowOpen = openRows.has(call.id); return <div key={call.id}>
          <button type="button" aria-expanded={rowOpen} onClick={() => toggleRow(call.id)} className="group flex h-6 w-full min-w-0 items-center gap-1.5 rounded-[4px] px-1 text-left transition-colors hover:bg-hover-2">
            <span className="flex size-3.5 shrink-0 items-center justify-center"><ToolIcon tool={call.tool} failed={call.status === 'failed'} /></span>
            <span className="w-10 shrink-0 text-[10.5px] font-medium text-ink-2">{call.tool}</span>
            <span className="min-w-0 flex-1 truncate font-mono text-[10.5px] text-ink-3">{call.summary}</span>
          ...
          <div className="grid transition-[grid-template-rows,opacity] duration-200" style={{ gridTemplateRows: rowOpen ? '1fr' : '0fr', opacity: rowOpen ? 1 : 0 }}><div className="min-h-0 overflow-hidden"><div className="my-0.5 ml-5 flex flex-col border-l border-line py-0.5 pl-2.5 font-mono text-[10px] leading-[1.55] text-ink-2">{call.detail?.map((line) => <span key={line} className={call.status === 'failed' ? 'text-red' : ''}>{line}</span>)}</div></div></div>
        </div> })}
      </div>
      {files.length > 0 && <div className="mt-1.5 flex flex-wrap gap-1">{files.map((file) => <button key={file.path} type="button" className="inline-flex h-5.5 max-w-full items-center gap-1 rounded-[4px] bg-field px-1.5 font-mono text-[9.5px] text-ink-2 shadow-hairline transition-colors hover:bg-hover"><span className="truncate">{file.path}</span><span className="text-green">+{file.additions}</span>{file.deletions > 0 && <span className="text-red">−{file.deletions}</span>}</button>)}</div>}
    </div></div>
  </div>
}
