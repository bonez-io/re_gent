...
export function FilesBlame({ lines, selectedHash = '7ac3ef1' }: FilesBlameProps) {
  return <div className="grid min-h-0 flex-1 grid-cols-[220px_minmax(0,1fr)] max-md:grid-cols-1">
    <aside className="border-r border-line bg-canvas max-md:hidden" aria-label="Changed files">
      <div className="flex h-9 items-center border-b border-line px-2.5 font-mono text-[10px] font-semibold uppercase">Files <span className="ml-auto font-normal text-ink-3">7 changed</span></div>
      <div className="py-1">{files.map(([file, stat], index) => <button key={`${file}-${index}`} className={`flex h-6.5 w-full items-center gap-1.5 border-l-2 px-2.5 text-left font-mono text-[10px] ${file === 'parser.ts' ? 'border-accent bg-hover-2 text-ink' : 'border-transparent text-ink-3 hover:bg-hover'}`}><span className={stat ? 'ml-3' : ''}>{stat ? '·' : '⌄'}</span><span className="min-w-0 flex-1 truncate">{file}</span><span className="text-[9px] text-green">{stat}</span></button>)}</div>
    </aside>
    <section className="min-w-0 bg-inset">
      ...
      <div className="mx-3 mt-4 overflow-hidden rounded-[4px] border border-line bg-canvas">
        <div className="flex h-8 items-center border-b border-line px-2.5 font-mono text-[10px] uppercase"><span className="font-semibold">Provenance</span><span className="ml-auto text-accent-ink">7ac3ef1</span></div>
